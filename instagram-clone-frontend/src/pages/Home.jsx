...
const Home = () => {
    ...
    const observer = new IntersectionObserver(
        ...
        { threshold: 0.9 } // 70% visible = active
    );

    // ✅ Take a snapshot of refs at this moment
    ...
    return () => {
        // ✅ Cleanup uses the snapshot, not the live ref object
        refsSnapshot.forEach((ref) => {
        if (ref) observer.unobserve(ref);
        });
    };
    }, [posts]); // ✅ posts dependency is enough
    ...
    return (
        <div className={`flex flex-col items-center min-h-screen p-4 transition-all duration-300 ${theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-black"}`}>
            ...
            {isFetchingPosts ? (
                ...
                posts.map((post) => {
                    const {
                        id, username, createdAt, mediaData,
                        mediaType, caption, likesCount,
                        commentsCount, profileImage
                    } = post;

                    const likeInfo = likesByPost[id] ?? { liked: false, count: likesCount ?? 0 };
                    
                    return (
                        <div key={id} className={`w-full max-w-md p-5 mb-6 rounded-lg shadow-lg ${theme === "dark" ? "bg-gray-800 border border-gray-700" : "bg-gray-100 border border-gray-300"}`}>
                            <div className="flex justify-between items-center mb-3">
                                <div className="flex items-center space-x-3">
                                    <img
                                        src={profileImage ? `data:image/png;base64,${profileImage}` : "placeholder.png"}
                                        alt="Profile"
                                        className="w-10 h-10 rounded-full border border-gray-400"
                                    />
                                    <Link to={`/${username}`} className="font-semibold">{username}</Link>
                                </div>
                            ...
                            <div className="w-full mb-3 relative">
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-2 right-2">
                                        ...
                                        isActive={currentPlayingPostId === String(id)} // ✅ only active plays
                                        muted={globalMuted} // ✅ pass global mute state
                                        setMuted={setGlobalMuted} // ✅ toggle globally
                                        />
                                    </div>
                                    )}
                                </div>
                            ...
                            />
                                ...
                                <button onClick={() => toggleCommentsPopup(id)}>{commentsCount} Comments</button>
                                <button>Share</button>
                            </div>
                            ...
                            {showCommentsPopup === id && (
                                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md max-h-[70vh] flex flex-col relative overflow-hidden">
                                        <button onClick={() => setShowCommentsPopup(null)} className="absolute top-2 right-3 text-red-500 text-xl">×</button>
